...
name: add-task
description:
  Use when adding or scaffolding a new Celery task in any app/tasks/ module.
...
- **Max retries:** 3 for most tasks, 5 for vector index tasks, `None` for
  finalization
...
Read the full target file to match the Celery app instance name, import style,
and session handling pattern.
...
Read `docs/tasks/tasks.md` and add a row for the new task: name, module, queue,
trigger, on-failure behaviour.
