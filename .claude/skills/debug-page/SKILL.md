---
name: debug-page
description: Use when investigating a page that is stuck, failed, or produced unexpected OCR output. Fetches and formats the full pipeline state for that page from Postgres.
...
If the user provided a filename, first query `documents` to find the `document_id`, then list its pages so the user can pick one.
...
```
Print as a compact table. If no blocks exist and status is `done`, flag: "Page marked done but no blocks found — possible `embed_and_index` failure."
