---
name: refactor-code
description: Rules and practices for safely handling non-trivial refactors in this repository. Use when restructuring shared FastAPI, Celery, pipeline, provider, config, schema, or indexing behavior; changing public interfaces or defaults; or moving code across modules with cross-cutting impact.
---

Use for any change that:
- Restructures shared code in `app/api/`, `app/tasks/`, `app/pipeline/`, `app/providers/`, `app/registry.py`, or `app/config.py`
- Changes public request/response schemas, task signatures, config keys, defaults, or status transitions
- Moves or renames modules used across multiple packages
- Changes hybrid search, chunking, arbitration, provider selection, or indexing behavior
- Requires coordination between code, tests, and docs

Do not use for: doc-only changes, typo fixes, or narrow single-file edits with no shared-behavior risk.
...
2. Run it and confirm it passes (behavior preservation) or fails for the expected reason (bug fix).
...
For non-trivial refactors, write a brief plan before broad edits. The plan should:
...
Extra care is required in these areas because they have hidden coupling across the system:
...
- If you discover a separate issue, note it separately instead of silently expanding scope.

## Public API Changes

If the refactor changes a public method signature, schema, task name, status value, or config key:
- Check all call sites before renaming.
- Prefer adding the new form and deprecating the old one if call sites are numerous.
- Document the migration path in the plan and update docs in `docs/` or `.env.example` as needed.
...
- Do not change embedding defaults or vector metadata assumptions without accounting for full reindex requirements.
- Do not remove task pre-condition checks; idempotency guards are part of behavior, not incidental implementation.
...
4. If API, config, or workflow expectations changed, update the relevant docs and `.env.example`.
5. If frontend-visible behavior changed and the app is runnable, run the narrowest relevant browser check.

Do not call the refactor complete if the protecting test story, validation, and docs are out of sync.
